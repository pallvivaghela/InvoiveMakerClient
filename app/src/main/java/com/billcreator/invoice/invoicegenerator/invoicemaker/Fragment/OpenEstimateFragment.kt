...
class OpenEstimateFragment : Fragment(), ModelChangeListener {
    ...
    private fun loadData() {
        catalogDTOS!!.clear()
        estimates?.let { catalogDTOS!!.addAll(it) }
    ...
    private fun updateEstimatesFromDatabase() {
        estimates = instance!!.allEstimates
        for (i in estimates!!.indices) {
            if (estimates!![i].estimateStatus == 2) {
    ...
    private fun initLayout() {
        ...
        invoiceMessage!!.text =
            mActivity!!.resources.getString(R.string.open_estimate_message)
    }
